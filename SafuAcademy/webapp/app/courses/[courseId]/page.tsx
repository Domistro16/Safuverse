...
export default function CourseDetailPage() {
  ...
  const courseKeyTakeaways = backendCourse?.keyTakeaways || [];
  const isCourseIncentivized = backendCourse?.isIncentivized ?? false;
  return (
    <Layout>
      <div className="space-y-6">
        ...
        <section className="grid lg:grid-cols-[minmax(0,3fr)_minmax(260px,1.5fr)] gap-6 items-start">
          ...
          <div className="space-y-4">
            {/* Video Player / SCORM Course */}
            {isEnrolled && isCourseIncentivized ? (
              /* Incentivized (SCORM) course - show SCORM course link */
              <div className={`aspect-video rounded-3xl relative overflow-hidden flex flex-col items-center justify-center gap-4 ${isDark
                ...
                <Link
                  href={`/courses/${courseId}/scorm`}
                  className={`z-10 px-6 py-3 rounded-2xl font-semibold text-[15px] transition shadow-xl ${isDark
                    ? 'bg-[#ffb000] text-black hover:bg-[#e6e200]'
                    : 'bg-safuDeep text-[#fef3c7] hover:bg-[#333]'
                    }`}
                >
                  Open SCORM Course
                </Link>
                {backendCourse?.scormVersion && (
                  <span className="z-10 px-3 py-1 rounded-full bg-black/60 text-[11px] text-blue-300">
                    {backendCourse.scormVersion === "SCORM_2004" ? "SCORM 2004" : "SCORM 1.2"}
                  </span>
                )}
        ...
        {/* Key Takeaways */}
        {courseKeyTakeaways.length > 0 && (
          <section className={`rounded-3xl border p-5 ${isDark ? 'border-[#2a2a3a] bg-[#12121a]' : 'border-black/5 bg-white/90 shadow-[0_4px_24px_rgba(0,0,0,0.06)]'}`}>
            <h2 className={`text-[18px] font-semibold mb-4 ${isDark ? 'text-white' : 'text-safuDeep'}`}>Key Takeaways</h2>
