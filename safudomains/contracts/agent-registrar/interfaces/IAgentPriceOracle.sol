...
interface IAgentPriceOracle {
     ...
     * @param name The name to check (without .safu suffix)
     * @return AgentPrice struct with pricing details
     */
    function getPrice(string calldata name) external view returns (AgentPrice memory);
     ...
     */
    function getPatternMatchCount(string calldata name) external view returns (uint256);
}
