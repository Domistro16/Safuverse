...
describe('NameWrapper', () => {
  ...
  describe('NameWrapper.names preimage dictionary', () => {
    it('Does not allow manipulating the preimage db by manually setting owner as NameWrapper', async () => {
      ...
      const subname1 = `${sublabel1}.${name}` // sub1.base.id
      ...
      const subname2 = `${sublabel2}.${subname1}` // sub2.sub1.base.id
      ...
      const sublabel3 = 'id'
      const subname3 = `${sublabel3}.${subname2}` // id.sub2.sub1.base.id
  ...
  describe('Grace period tests', () => {
    ...
    async function gracePeriodFixture() {
      ...
      // move .id name to expired and be within grace period
    ...
    it('When a .id name is in grace period it cannot call setSubnodeOwner', async () => {
    ...
    it('When a .id name is in grace period it cannot call setSubnodeRecord', async () => {
      const { nameWrapper, parentExpiry, accounts } = await loadFixture(
        gracePeriodFixture,
      )
    ...
    it('When a .id name is in grace period it cannot call setRecord', async () => {
    ...
    it('When a .id name is in grace period it cannot call safeTransferFrom', async () => {
      const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture)
    ...
    it('When a .id name is in grace period it cannot call batchSafeTransferFrom', async () => {
      const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture)
    ...
    it('When a .id name is in grace period it cannot call setResolver', async () => {
      const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture)
    ...
    it('When a .id name is in grace period it cannot call setTTL', async () => {
      const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture)
    ...
    it('When a .id name is in grace period it cannot call setFuses', async () => {
      const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture)
    ...
    it('When a .id name is in grace period it cannot call setChildFuses', async () => {
      const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture)
    ...
    it('When a .id name is in grace period, unexpired subdomains can call setFuses', async () => {
      const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture)
    ...
    it('When a .id name is in grace period, unexpired subdomains can transfer', async () => {
      const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture)
    ...
    it('When a .id name is in grace period, unexpired subdomains can set resolver', async () => {
    ...
    it('When a .id name is in grace period, unexpired subdomains can set ttl', async () => {
      const { ensRegistry, nameWrapper, accounts } = await loadFixture(
        gracePeriodFixture,
      )
    ...
    it('When a .id name is in grace period, unexpired subdomains can call setRecord', async () => {
      const { ensRegistry, nameWrapper, accounts } = await loadFixture(
        gracePeriodFixture,
      )
    ...
    it('When a .id name is in grace period, unexpired subdomains can call setSubnodeOwner', async () => {
    ...
    it('When a .id name is in grace period, unexpired subdomains can call setSubnodeRecord', async () => {
      const { nameWrapper, actions, accounts } = await loadFixture(
        gracePeriodFixture,
      )
    ...
    it('When a .id name is in grace period, unexpired subdomains can call setChildFuses if the subdomain exists', async () => {
      const { nameWrapper, actions, accounts } = await loadFixture(
        gracePeriodFixture,
      )
  ...
  describe('Registrar tests', () => {
    ...
    it('Reverts when attempting to call token owner protected function on an unwrapped name', async () => {
      ...
      // XXX: note that at this step, the hackler should use the current .id
      // registrar to directly register `sub1.id` to himself, without wrapping
      // the name.
      await actions.register({
        label,
        owner: accounts[2].address,
      ...
      //
      // XXX: corrently, `sub1.id` becomes a normal node
      await ensRegistry.write.setOwner([namehash(name), nameWrapper.address], {
        account: accounts[2],
      })

      // create `sub2.sub1.id` to the victim user with `PARENT_CANNOT_CONTROL`
      // burnt.
      await expect(nameWrapper)
        .write(
          'setSubnodeOwner',
  ...
  describe('ERC1155 additional tests', () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap the .id name', async () => {
      const { nameWrapper, actions, accounts } = await loadFixture(fixture)
      ...
      await expect(nameWrapper)
        ...
        .toBeRevertedWithCustomError('Unauthorised')
        .withArgs(namehash(label + '.id'), getAddress(accounts[2].address))
    })

    it('Approval on the Wrapper does not give permission to wrap a non .id name', async () => {
    ...
    it('When .id name expires, it is untransferrable', async () => {
  ...
  describe('Implicit unwrap tests', () => {
    ...
    it('Trying to burn child fuses when re-registering a name on the old controller reverts', async () => {
      ...
      // create `sub2.sub1.id` w/o fuses burnt
      await actions.setSubnodeOwner.onNameWrapper({
        parentName: name,
        label: sublabel,
        owner: accounts[2].address,
      ...
      // XXX: note that at this step, the hacker should use the current .id
      // registrar to directly register `sub1.id` to themselves, without wrapping
      // the name.
      await actions.register({
        label,
        owner: accounts[2].address,
      ...
      // XXX: PREPARE HACK!
      // set `EnsRegistry.owner` of `sub1.id` as the hacker themselves.
      ...
      // set `sub2.sub1.id` to the victim user w fuses burnt
    ...
    it('Renewing a wrapped, but expired name .id in the wrapper, but unexpired on the registrar resyncs expiry', async () => {
