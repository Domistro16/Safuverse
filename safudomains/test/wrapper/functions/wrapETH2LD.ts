...
export const wrapETH2LDTests = () =>
  describe('wrapETH2LD()', () => {
    ...
    it('Cannot wrap a name if the owner has not authorised the wrapper with the .id registrar.', async () => {
    ...
    it('Can re-wrap a name that was wrapped has already expired on the .id registrar', async () => {
    ...
    it('Allows an account approved by the owner on the .id registrar to wrap a name.', async () => {
      const { baseRegistrar, nameWrapper, accounts, actions } =
        await loadFixture(fixture)
